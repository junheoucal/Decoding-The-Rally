...
OUT_PATH = SCRIPT_DIR / "vren_edgelist_hitter.csv"


def main() -> None:
    df = pd.read_csv(DATA_PATH)

    loc = df["set_location"].notna() & df["hit_land_location"].notna()
    ...
    out = (
        tmp.groupby(["hitter", "hit_land_location"], as_index=False)
        .agg(
            in_denom=("_in", "sum"),
            in_num=("_succ_in", "sum"),
            out_denom=("_out", "sum"),
